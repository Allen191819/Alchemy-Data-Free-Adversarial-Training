...
def create_attack(model, criterion, attack_type, attack_eps, attack_iter=10, attack_step=0.007, rand_init_type='uniform', 
                  clip_min=0., clip_max=1.,device=None):
    """
   ...
   """
    ...
    if attack_type == 'fgsm':
        attack = FGSMAttack(model, criterion, eps=attack_eps, clip_min=clip_min, clip_max=clip_max,device=device)
    elif attack_type == 'fgm':
        attack = FGMAttack(model, criterion, eps=attack_eps, clip_min=clip_min, clip_max=clip_max,device=device)
    elif attack_type == 'linf-pgd':
        attack = LinfPGDAttack(model, criterion, eps=attack_eps, nb_iter=attack_iter, eps_iter=attack_step,
                               rand_init_type=rand_init_type, clip_min=clip_min, clip_max=clip_max,device=device)
        ...
        attack = LinfDeepFoolAttack(model, overshoot=0.02, nb_iter=attack_iter, search_iter=0, clip_min=clip_min, eps=attack_eps,
                                    clip_max=clip_max,device=device)
    elif attack_type == 'l2-df':
        attack = L2DeepFoolAttack(model, overshoot=0.02, nb_iter=attack_iter, search_iter=0, clip_min=clip_min, eps=attack_eps,
                                  clip_max=clip_max,device=device)
    elif attack_type == 'linf-apgd':
        attack = LinfAPGDAttack(model, criterion, n_restarts=2, eps=attack_eps, nb_iter=attack_iter,device=device)
    elif attack_type == 'l2-apgd':
        attack = L2APGDAttack(model, criterion, n_restarts=2, eps=attack_eps, nb_iter=attack_iter,device=device)
    elif attack_type == 'fab':
        attack = FAB(model,n_restarts=2,nb_iter=attack_iter,eps=attack_eps,device=device)
    elif attack_type == 'linf-spsa':
        attack = LinfSPSA(model,eps=attack_eps,nb_iter=attack_iter,device=device)
    else:
