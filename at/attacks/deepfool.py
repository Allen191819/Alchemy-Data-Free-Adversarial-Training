...
def perturb_deepfool(xvar, yvar, predict, nb_iter=50, overshoot=0.02, ord=np.inf, clip_min=0.0, clip_max=1.0, 
                     search_iter=0,eps=8/255, device=None):
    """
    ...
    for i in range(nb_iter):
        ...
        delta = (1. + overshoot) * r_tot
        x.data = clamp_by_pnorm(delta.data, ord, eps) + x_orig
...
class DeepFoolAttack(Attack, LabelMixin):
    ...
    def __init__(
            self, predict, overshoot=0.02, nb_iter=50, search_iter=50, clip_min=0., clip_max=1., ord=np.inf,eps=8/255,device=None):
        ...
        self.ord = ord
        print(f"Eps of deepfool {self.eps}")
    ...
    def perturb(self, x, y=None):
        ...
        x_adv, r_adv = perturb_deepfool(x, y, self.predict, self.nb_iter, self.overshoot, ord=self.ord, 
                                        ...
                                        eps=self.eps,device=self.device)
        return x_adv, r_adv
...
class LinfDeepFoolAttack(DeepFoolAttack):
    ...
    def __init__(
            self, predict, overshoot=0.02, nb_iter=50, search_iter=50, clip_min=0., clip_max=1.,eps=8/255,device=None):
        ...
        super(LinfDeepFoolAttack, self).__init__(
            ...
            clip_max=clip_max, ord=ord,eps=eps,device=device)
...
class L2DeepFoolAttack(DeepFoolAttack):
    ...
    def __init__(
            self, predict, overshoot=0.02, nb_iter=50, search_iter=50, clip_min=0., clip_max=1.,eps=16/255,device=None):
