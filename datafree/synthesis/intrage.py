...
class NeoDFATSynthesizer(BaseSynthesis):
    def __init__(self, teacher,student, generator, nz, num_classes, img_size, 
        ...
        self.optimizer_head = torch.optim.Adam(self.head.parameters(), lr=self.lr_h)
    ...
    def synthesize(self, targets=None):
        ...
        for it in tqdm(range(self.iterations)):
            ...
            if self.alpha > 0:
                if self.softmax:
                    loss_mm = max_margin_loss( F.softmax(t_out,dim=1), targets, threshold=self.threshold )
                else:
                    loss_mm = max_margin_loss( t_out, targets,threshold=self.threshold)
            else:
                loss_mm = loss_oh.new_zeros(1)
            ...
            loss_inv = self.bn * loss_bn + self.oh * loss_oh +  self.alpha * loss_mm + self.adv * loss_adv
            ...
            # print(f"loss_bn {loss_bn}")
            # print(f"loss_oh {loss_oh}")
            # print(f"loss_adv {loss_adv}")
            # print(f"loss_mm {loss_mm}")
            # print(f"loss_cr {loss_cr}")
            # print(f"Loss: {loss}")
            # print(f"{self.bn}*{loss_bn}+{self.oh}*{loss_oh}+{self.adv}*{loss_adv}+{self.alpha}*{loss_mm}+{self.cr}*{loss_cr}")
            with torch.no_grad():
