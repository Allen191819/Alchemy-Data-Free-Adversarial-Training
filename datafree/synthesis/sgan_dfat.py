...
class DFATSGanSynthesizer(BaseSynthesis):
    def __init__(self, model, nz, num_classes, img_size, 
                 ...
                 autocast=None, use_fp16=False, debug=False,pretrained=False,
                 normalizer=None, device='cpu', distributed=False):
        super(DFATSGanSynthesizer, self).__init__(model,model)
        ...
        self.generator = datafree.models.stylegan_generator.StyleGANGenerator(resolution=self.img_size[1],
                 z_space_dim=nz,
                 w_space_dim=512,
                 label_size=0,
                 mapping_layers=8,
                 ...
                 use_wscale=True,
                 fmaps_base=16 << 10,
                 fmaps_max=512).to(device).train()
        if pretrained:
            checkpoint = torch.load("./checkpoints/generators/SGan.pth", map_location=device)
    ...
    def train_generator(self):
        ...
        optim_G = torch.optim.Adam([{'params': self.generator.parameters()}], self.lr_g, betas=[0.5, 0.999])
        scheduler_G = torch.optim.lr_scheduler.ExponentialLR(optim_G, gamma=0.99)
        self.teacher.eval()
        self.generator.train()
        for it in tqdm(range(self.iterations_g)):
            targets = torch.randint(low=0, high=self.num_classes, size=(self.synthesis_batch_size,)).to(self.device)
            z = torch.randn(size=(self.synthesis_batch_size,self.nz),device=self.device)
            inputs = self.generator(z,label = targets)
            global_view = self.normalizer(inputs) # crop and normalize
            t_out = self.teacher(global_view)
            # loss_bn = sum([h.r_feature for h in self.hooks])
            # loss_oh = cross_entropy(t_out,targets, reduction="mean")
            ...
            if self.adv>0:
                ...
                adv_global_view = self.normalizer(adv_inputs)
                adv_out = self.teacher(adv_global_view)
                loss_adv = cross_entropy(adv_out, t_out.max(1)[1], reduction='mean')
            else:
                loss_adv = loss_mm.new_zeros(1)

            loss = loss_mm*self.alpha # loss_oh*self.oh + loss_adv*self.adv + loss_bn*self.bn 
            optim_G.zero_grad()
            loss.backward()
            optim_G.step()
            scheduler_G.step()
            _,pred = t_out.max(1)
            if self.debug and it%100==0:
                self.data_pool.add( inputs, target=targets)
                print(f"({it}/{self.iterations_g}) :==: Correct ==> ({(pred == targets).sum().item()}/{t_out.shape[0]}) Loss:{loss.item():.4}")
            if it%100==0 and it!= 0:
                os.makedirs("./checkpoints/generators",exist_ok=True)
                torch.save({'state_dict': self.generator.state_dict()},"./checkpoints/generators/SGan.pth")
        print(f"Finished training generator")
    ...
    def synthesize(self, targets=None):
        ...
        # reset_model(self.generator)
        optimizer = torch.optim.Adam([{'params': generator.parameters()}, {'params': [z]}], self.lr_g, betas=[0.5, 0.999])
        ...
        for it in tqdm(range(self.iterations)):
            inputs = generator(z,label = targets)
        ...
        conf,pred = self.teacher(self.normalizer(best_inputs)).max(1)
        print(f"Correct: {(pred == targets).sum().item()}/{pred.shape[0]},Conf Mean: {conf.mean()}")
