...
def save_image_batch(imgs, output, col=None, size=None, pack=True, target=None, batch_n=None, current_time='',dataset='cifar10'):
    ...
    if pack:
        if dataset=='mnist':
            imgs = pack_images( imgs, col=col ).squeeze()
        else:
        ...
        if target is None:
            ...
            for idx, (t, img) in enumerate(zip(target,imgs)):
                os.makedirs(os.path.join(output,f"class_{t:02d}"),exist_ok=True)
                if dataset=='mnist':
                    img = Image.fromarray( img.squeeze() )
                else:
                    img = Image.fromarray( img.transpose(1, 2, 0) )
                img.save(os.path.join(output,f"class_{t:02d}",current_time+f"{batch_n}_{idx}.png"))
...
class UnlabeledGrayImageDataset(torch.utils.data.Dataset):
...
class LabeledGrayImageDataset(torch.utils.data.Dataset):
    def __init__(self, root, transform=None):
        self.root = os.path.abspath(root)
        self.folders = os.listdir( root )
        self.categories = [int(cls[-2:]) for cls in self.folders]
        images = []
        targets = []
        for c,folder in zip(self.categories,self.folders):
            category_dir = os.path.join( self.root, str(folder))
        ...
        self.transform = transform
    def __getitem__(self, idx):
        img, target = Image.open( self.images[idx] ).convert('L'), self.targets[idx]
        if self.transform:
            img = self.transform(img)
        return img, target
    def __len__(self):
...
class ImagePool(object):
    ...
    def add(self, imgs, target=None):
        if target is None:
            save_image_batch(imgs, os.path.join( self.root, "%d.png"%(self._idx) ), pack=False, dataset=self.dataset)
        elif isinstance (target,int):
            os.makedirs(os.path.join(self.root,f"class_{target:02d}"),exist_ok=True)
            save_image_batch(imgs, os.path.join( self.root, f"class_{target:02d}","%d.png"%(self._idx) ), pack=False, dataset=self.dataset)
        else:
            save_image_batch(imgs, self.root, pack=False, target=target,batch_n=self._idx, current_time=self._current_time, dataset=self.dataset)
        self._idx+=1

    def get_dataset(self, transform=None, labeled=True):
        if self.dataset in ['cifar10','cifar100','svhn','cinic10']:
            if labeled:
                return datasets.ImageFolder(self.root, transform=transform)
            else:
                return UnlabeledImageDataset(self.root, transform=transform)
        else:
            if labeled:
                return LabeledGrayImageDataset(self.root, transform=transform)
            else:
                return UnlabeledGrayImageDataset(self.root, transform=transform)
